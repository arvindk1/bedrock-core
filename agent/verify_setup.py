...
async def run_verification():
    ...
    for symbol in symbols:
        ...
        if earnings:
            logger.warning(f"⚠️  Earnings Warning: {earnings['warning']} on {earnings['earnings_date']}")
        else:
        ...
        vol_res = await vol_calc.calculate_volatility(symbol, model=VolatilityModel.HYBRID)
        ...
        for trade in opportunities[:2]: # Check top 2
            ...
            if score_card.is_approved:
                ...
                logger.info(f"   ❌ REJECTED (Score: {score_card.total_score:.1f}) - {score_card.rejection_reason}")
