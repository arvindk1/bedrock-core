...
class MarketData:
    ...
    def get_option_chain(self, symbol: str, expiration_date: Optional[str] = None) -> Optional[Any]:
        """
        ...
        try:
            ...
            if expiration_date:
                if expiration_date not in expirations:
                     logger.warning(f"Expiration {expiration_date} not found for {symbol}. Available: {expirations[:3]}...")
                     return None
    ...
    def calculate_relative_strength(self, symbol: str, benchmark: str = "SPY", period_days: int = 20) -> float:
        """
        ...
        try:
            ...
            # Download data
            data = yf.download([symbol, benchmark], start=start_date, progress=False)["Close"]
            
            if data.empty or symbol not in data.columns or benchmark not in data.columns:
                return 0.0
            ...
            # Simple RS Ratio
            rs_score = data[symbol] / data[benchmark]
    ...
    def get_liquidity_metrics(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            return {"avg_volume": 0, "current_volume": 0, "stock_spread": 0, "spread_pct": 0}
