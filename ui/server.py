...
async def serve_js():
    return FileResponse(
        os.path.join(UI_DIR, "app.js"), media_type="application/javascript"
    )
...
async def smart_scan(req: SmartScanRequest):
    ...
    try:
        ...
        opportunities = await options_scanner.scan_opportunities(
            symbol=req.symbol, strategy_preference=req.strategy_preference
        )
        ...
        return {
            ...
            if opportunities
            else "No opportunities found matching criteria.",
        }
    except Exception as e:
        logger.error(f"Scan failed: {e}", exc_info=True)
        return JSONResponse(
            status_code=500, content={"status": "error", "output": str(e)}
        )
...
def scan(req: ScanRequest):
    ...
    """
    logger.info(
        f"Received dashboard scan for {req.symbol} ({req.start_date} to {req.end_date})"
    )
    try:
        ...
        policy_amount = policy_amounts.get(
            req.policy_mode, APP_CONFIG["policy_limits"]["tight"]
        )
        ...
        corr_rejs_enriched = _enrich_rejections(
            log_dict.get("correlation_rejections", [])
        )

        # No-trades explanation (from orchestrator — populated when picks = 0)
        no_trades_explanation = None
        if (
            hasattr(decision_log, "no_trades_explanation")
            and decision_log.no_trades_explanation
        ):
        ...
        return {
            ...
            "volatilityContext": {
                ...
                "expected_move_30d": log_dict.get("regime_details", {}).get(
                    "expected_move"
                ),
            },
            "decisionLog": {
...
def get_portfolio_risk():
    ...
    try:
        ...
        total_capital_at_risk = sum(abs(p.cost_basis * p.quantity) for p in positions)

        # --- Compute net_delta (per share * 100 shares per contract) ---
        net_delta = sum((p.delta or 0.0) * p.quantity * 100 for p in positions)

        # --- Compute daily_drawdown ---
        daily_drawdown = sum((p.unrealized_pnl or 0.0) for p in positions)

        # --- Compute sector_exposure ---
        sector_risk: dict = {}
        for p in positions:
            sector = p.sector or "Unknown"
            sector_risk[sector] = sector_risk.get(sector, 0.0) + abs(
            ...
            result.append(
                {
                    ...
                    "current_mark": round(float(p.current_mark), 2)
                    if p.current_mark is not None
                    else None,
                    "unrealized_pnl": round(float(p.unrealized_pnl), 2)
                    if p.unrealized_pnl is not None
                    else None,
                    "max_profit": round(float(p.max_profit), 2)
                    if p.max_profit is not None
                    else None,
                    "max_loss": round(float(p.max_loss), 2)
...
async def get_config():
    ...
    return {
        ...
        "risk_limits": {
            "max_sector_concentration_pct": APP_CONFIG["risk_limits"][
                "max_sector_concentration_pct"
            ],
            "max_portfolio_correlation": APP_CONFIG["risk_limits"][
...
def trade_execute(req: TradeExecuteRequest):
    ...
    try:
        ...
        sector = (
            ...
            else SECTOR_MAP.get(req.symbol, "Unknown")
        )
